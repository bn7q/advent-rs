use crate::{puzzle::Puzzle, utils::{count_elements, read_grid_parsed}};
...
impl Puzzle for P {
    fn solve1(&self, input: &str) -> String {
        ...
        let (mut list1, mut list2): (Vec<i32>, Vec<i32>) = grid.iter().map(|v| (v[0], v[1])).unzip();

        list1.sort();
        list2.sort();

        let result: i32 = list1.iter().zip(list2)
            .map(|(l, r)| (l - r).abs() )
            .sum();
    ...
    fn solve2(&self, input: &str) -> String {
        ...
        let result: u64 = map1.iter().map(|(key, count1)| {
            let x = map2.get(key).unwrap_or(&0) * key * count1;
            x
        })
        .sum();
